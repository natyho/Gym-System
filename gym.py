...
class Gym:
    ...
    def schedule_workout_class(self, time_point: datetime, room_name: str,
        ...
        True
        """
        if not (self._instructors[instr_id].can_teach(self._workouts[workout_name])):
            return False
        if time_point in self._schedule:
            if room_name in self._schedule[time_point]:
                return False
            for r in self._schedule[time_point]:
                if instr_id == self._schedule[time_point][r][0].get_id():
                    return False
    ...
    def register(self, time_point: datetime, client: str, workout_name: str) \
        ...
        for room in self._schedule[time_point]:
            if client in self._schedule[time_point][room][2]:
                return False
        for room in self._schedule[time_point]:
            if self._schedule[time_point][room][1].get_name() == workout_name:
                if len(self._schedule[time_point][room][2]) < self._rooms[room]:
    ...
    def offerings_at(self, time_point: datetime) -> List[Tuple[str, str, str]]:
        ...
        if time_point not in self._schedule:
            ...
            for room in self._schedule[time_point]:
                n = self._schedule[time_point][room][0].name
                w = self._schedule[time_point][room][1].get_name()
                offerings.append((n, w, room))
            return offerings
        ...
        for time in self._schedule:
            if time1 <= time <= time2:
                for room in self._schedule[time]:
                    if self._schedule[time][room][0].get_id() not in hours:
                        ins_id = self._schedule[time][room][0].get_id()
                        hours[ins_id] = 0
                    hours[self._schedule[time][room][0].get_id()] += 1
        for instructor_id in self._instructors:
            if instructor_id not in hours:
    ...
    def payroll(self, time1: datetime, time2: datetime, base_rate: float) \
        ...
        """
        total_pay = []
        for ins_id in self.instructor_hours(time1, time2):
            name = self._instructors[ins_id].name
            hour = self.instructor_hours(time1, time2)[ins_id]
            bonus = self._instructors[ins_id].get_num_certificates()
            total_pay.append((ins_id, name, hour, hour*(base_rate + bonus*BONUS_RATE)))
        total_pay.sort()
        return total_pay
